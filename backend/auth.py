...
try:
    ...
    class jwt:
        ...
        def encode(claims: dict, key: str, algorithm: str) -> str:
            ...
            payload = base64.urlsafe_b64encode(json.dumps(claims, default=str).encode()).decode().rstrip('=')
...
class AuthConfig:
    ...
    def __init__(self):
        ...
        self.skip_verification = os.getenv("SKIP_JWT_VERIFICATION", "false").lower() == "true"
...
async def get_current_user(
    ...
    user_data = {
        "sub": token_data.get("sub") or token_data.get("uid") or token_data.get("user_id"),
        "email": token_data.get("email") or token_data.get("email_verified"),
        "name": token_data.get("name") or token_data.get("given_name", "") + " " + token_data.get("family_name", ""),
